...
from helpers.helper import initialize_edit, initialize_text, initialize_widget, initialize_combo, update_combo
...
class UIWindow(Protocol):
    def update_point_buys(self, attribute: str=None, attribute_value: int=None) -> None:
        ...

    def update_increase_buys(self, attribute: str=None, attribute_value: int=None) -> None:
        ...
...
class AbilityScore:
    ...
    def update_point_buy_spendable_points(self, point_buy_spendable_points: int):
        ...
        self.remaining_point_box.setText(str(point_buy_spendable_points))
    ...
    def update_ability_buy_spendable_points(self, ability_buy_spendable_points: int):
        ...
        self.remaining_ability_box.setText(str(ability_buy_spendable_points))
    ...
    def point_buys_triggered(self, triggered_by: str):
        """update abilities based on the point buy system
        """
        combos = self.get_ability_score_combos()
        triggered_combo = combos[triggered_by]

        self.parent.update_point_buys(triggered_by, int(triggered_combo.currentText()))


    def update_point_buys(self, spent_points_dict: dict=None):
        race_attributes = {
            "strength" : self.str_race,
            "dexterity" : self.dex_race,
            "constitution" : self.con_race,
        ...
        for attr, combo in self.get_ability_score_combos().items():
            spent_points = 0
            if spent_points_dict is not None:
                spent_points = spent_points_dict[attr]
            val = self.point_buy_spendable_points + spent_points + 1
            race_val = int(race_attributes[attr].text())
            theme_val = int(theme_attributes[attr].text())
            max_val = 10 + race_val + theme_val
            points = [str(i) for i in range(val) if i + max_val <= 18]
            update_combo(combo, points)
            combo.setCurrentIndex(spent_points)

    def increase_buys_triggered(self, triggered_by: str):
        combos = self.get_ability_increase_buy_combo()
        triggered_combo = combos[triggered_by]
        self.parent.update_increase_buys(triggered_by, int(triggered_combo.currentText()))

    def update_increase_buys(self, level, spent_points_dict: dict=None):
        for attr, combo in self.get_ability_increase_buy_combo().items():
            val = ((level // 5) * 1 if self.ability_buy_spendable_points > 0 else 0) + 1
            val = max(spent_points_dict[attr] + 1, val)
            points = [str(i) for i in range(val)]
            update_combo(combo, points)
            combo.setCurrentIndex(spent_points_dict[attr])

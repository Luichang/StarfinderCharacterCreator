...
class SkillPoints:
    ...
    def get_skill_rank_combos(self) -> dict[SKILL_NAMES, QtWidgets.QComboBox]:
        skill_buys = {
    ...
    def get_skill_bought_rank(self) -> dict[SKILL_NAMES, int]:
    ...
    def update_skill_buy_spendable_points(self, skill_buy_spendable_points: int, skill_buy_max_points: int):
        ...
        self.remaining_skill_box.setText(str(skill_buy_spendable_points))
        self.skills_per_box.setText(str(skill_buy_max_points))

    def update_rank_combos(self, level: int, character_skill_ranks: dict[str, int]):
        total_used = 0
        for skill, combo in self.get_skill_rank_combos().items():
            current_value = int(combo.currentText())
            if current_value > level:
                current_value = 0
            total_used += current_value
            update_combo(combo, [str(x) for x in range(level + 1)])
            combo.setCurrentIndex(character_skill_ranks[skill])
    ...
    def update_ability_modifier(self, skills):
    ...
    def update_skill_buy(self, changed_skill: SKILL_NAMES):
        skill_buys = self.get_skill_rank_combos()
        changed_value = int(skill_buys[changed_skill].currentText())
        self.parent.update_skill_buy(changed_skill, changed_value)
